...
<head>
  ...
  <title>Jetlouge Travels - Request Profile Update</title>
    ...
    <div class="row justify-content-center">
      <div class="col-12 col-lg-8">
        ...
        <div class="d-flex justify-content-between align-items-center mb-4">
          <div>
            <h2 class="mb-1 fw-bold">Request Profile Update</h2>
            <p class="text-muted mb-0">Submit a request to update your profile information</p>
          </div>
        ...
        <div class="card main-card">
          <div class="card-header card-header-custom">
            <h4 class="mb-0">
              <i class="bi bi-person-gear me-2"></i>Profile Update Request
            </h4>
          </div>
          <div class="card-body p-4">
            <form action="{{ route('employee.profile_updates.store') }}" method="POST" enctype="multipart/form-data" id="profileUpdateForm">
              @csrf
              ...
              <div class="mb-4">
                ...
                <select name="field_name" id="field_name" class="form-select @error('field_name') is-invalid @enderror" required>
                  ...
                  <option value="phone_number" {{ old('field_name') == 'phone_number' ? 'selected' : '' }}>Phone Number</option>
                  <option value="emergency_contact_name" {{ old('field_name') == 'emergency_contact_name' ? 'selected' : '' }}>Emergency Contact Name</option>
                  <option value="emergency_contact_phone" {{ old('field_name') == 'emergency_contact_phone' ? 'selected' : '' }}>Emergency Contact Phone</option>
                  <option value="emergency_contact_relationship" {{ old('field_name') == 'emergency_contact_relationship' ? 'selected' : '' }}>Emergency Contact Relationship</option>
                  <option value="profile_picture" {{ old('field_name') == 'profile_picture' ? 'selected' : '' }}>Profile Picture</option>
                </select>
                @error('field_name')
              ...
              <div class="mb-4" id="text-input-section">
                ...
                <input type="text" name="new_value" id="new_value" class="form-control @error('new_value') is-invalid @enderror" 
                       value="{{ old('new_value') }}" placeholder="Enter the new value">
                @error('new_value')
                  <div class="invalid-feedback">{{ $message }}</div>
                @enderror
              </div>
              ...
              <div class="mb-4">
                ...
                <textarea name="reason" id="reason" class="form-control @error('reason') is-invalid @enderror" 
                          rows="3" placeholder="Please provide a reason for this update request...">{{ old('reason') }}</textarea>
                @error('reason')
                  <div class="invalid-feedback">{{ $message }}</div>
                @enderror
              </div>
              ...
              <div class="field-info">
                ...
                <ul class="mb-0 small">
                  <li>All profile update requests require approval from HR</li>
                  <li>You will be notified via email once your request is processed</li>
                  <li>Only pending requests can be edited or cancelled</li>
