...
<body style="background-color: #f8f9fa !important;">
  ...
  <main id="main-content">
    ...
    <div class="simulation-card card mb-4">
      <div class="card-header card-header-custom d-flex justify-content-between align-items-center">
        <h4 class="fw-bold mb-0"><i class="bi bi-lightbulb me-2"></i>Leadgen - AI What-if Scenarios</h4>
        <div class="d-flex gap-2">
          <button class="btn btn-outline-success btn-sm" onclick="runAIScenarioWithConfirmation()">
            <i class="bi bi-play-fill me-1"></i>Run AI Simulation
          </button>
          <button class="btn btn-outline-primary btn-sm" onclick="generateCustomScenarioWithConfirmation()">
            <i class="bi bi-plus-lg me-1"></i>Custom Scenario
          </button>
          <button class="btn btn-outline-info btn-sm" onclick="exportScenarioReportWithConfirmation()">
  ...
  <script>
    ...
    // ===== ENHANCED AI-POWERED WHAT-IF SCENARIOS =====
    
    // Run AI Scenario with Confirmation
    async function runAIScenarioWithConfirmation() {
      const { value: password } = await Swal.fire({
        title: '<i class="bi bi-cpu text-success"></i> AI Scenario Analysis',
        html: `
          <div class="text-start">
            <p class="mb-3">Please enter your admin password to run AI-powered scenario analysis:</p>
            <div class="alert alert-info">
              <i class="bi bi-robot me-2"></i>
              <strong>AI Analysis:</strong> This will generate intelligent succession planning scenarios using machine learning algorithms and current organizational data.
            </div>
          </div>
        `,
        input: 'password',
        ...
        showCancelButton: true,
        confirmButtonText: '<i class="bi bi-play-fill me-1"></i>Run AI Analysis',
        cancelButtonText: '<i class="bi bi-x-lg me-1"></i>Cancel',
        confirmButtonColor: '#198754',
        cancelButtonColor: '#6c757d',
        width: '500px',
    ...
    // Advanced AI Scenario Generation
    async function runAdvancedAIScenario() {
      const scenarioType = document.getElementById('scenarioType').value;
      const impactSeverity = document.getElementById('impactSeverity').value;
      const timeline = document.getElementById('scenarioTimeline').value;
      
      Swal.fire({
        title: '<i class="bi bi-cpu text-primary"></i> AI Analysis in Progress',
        html: `
          <div class="text-center">
            <div class="spinner-border text-primary mb-3" role="status"></div>
            <p class="mb-2">🤖 <strong>AI Engine Processing...</strong></p>
            <p class="text-muted">Analyzing organizational data and generating intelligent scenarios</p>
        ...
        didOpen: () => {
          ...
          const interval = setInterval(() => {
            ...
            if (progress >= 100) {
              ...
              setTimeout(() => {
                Swal.close();
                generateAdvancedAIScenarios(scenarioType, impactSeverity, timeline);
              }, 500);
            }
          }, 300);
        }
